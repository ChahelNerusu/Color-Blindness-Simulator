...
BMP *bmp_create(FILE *fin) {
    ...
    uint32_t skip_uint32;
    uint16_t skip_uint16;
    uint8_t skip_uint8;
    ...
    read_uint32(fin, &skip_uint32);
    read_uint16(fin, &skip_uint16);
    read_uint16(fin, &skip_uint16);
    read_uint32(fin, &skip_uint32);
...
void bmp_reduce_palette(BMP *pbmp) {
    for (int i = 0; i < MAX_COLORS; i++) {
        ...
        if (SqLe < SeLq) {
            new_r = constrain(0.426331 * r + 0.875102 * g + 0.0801271 * b);
            new_g = constrain(0.281100 * r + 0.571195 * g - 0.0392627 * b);
            new_b = constrain(-0.0177052 * r + 0.0270084 * g + 1.00247 * b);
        } else {
            new_r = constrain(0.758100 * r + 1.45387 * g - 1.48060 * b);
            new_g = constrain(0.118532 * r + 0.287595 * g + 0.725501 * b);
            new_b = constrain(-0.00746579 * r + 0.0448711 * g + 0.954303 * b);
        }

        pbmp->palette[i].red = new_r;
        pbmp->palette[i].green = new_g;
        pbmp->palette[i].blue = new_b;
    }
}
